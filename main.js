...
const btnOpenApplicationInsurence = document.getElementById(
  "btn-insurance-application",
);
...
btnMenu.addEventListener("click", () => {
  ...
  if (menu.classList.contains("menu_show"))
    btnMenu.classList.add("header__burger-menu_click");
...
btnCancelApplication.addEventListener("click", (event) => {
  ...
  const inputs = windowApplication.getElementsByClassName(
...
setInterval(() => {
  for (let i = 0; i < collectionBackImg.length; i += 1) {
    if (
      collectionBackImg[i].classList.contains("why-us__back-img_animation") &&
      i < collectionBackImg.length - 1
    ) {
